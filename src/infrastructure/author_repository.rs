...
impl AuthorRepository for PgAuthorRepository {
    ...
    async fn restore(
        ...
        let es_id = Uuid::new_v4();
        sqlx::query(
            "INSERT INTO event_set (id, user_id, operation) VALUES ($1, $2, 'restore_author')",
        )
        .bind(es_id)
        .bind(user_id.as_str())
        .execute(&mut *conn)
...
mod tests {
    ...
    async fn create_and_find_all(pool: PgPool) -> anyhow::Result<()> {
        ...
        author_repository
            .create(&mut conn, &user_id, &author1)
            .await?;
        author_repository
            .create(&mut conn, &user_id, &author2)
            .await?;

        let all_authors = author_repository.find_all(&mut conn, &user_id).await?;
    ...
    async fn find_by_id_does_not_return_other_users_author(pool: PgPool) -> anyhow::Result<()> {
        ...
        author_repository
            .create(&mut conn, &user1_id, &author)
            .await?;

        let result = author_repository
            .find_by_id(&mut conn, &user2_id, &author_id)
            .await?;
    ...
    async fn find_all_does_not_return_other_users_authors(pool: PgPool) -> anyhow::Result<()> {
        ...
        let result = author_repository.find_all(&mut conn, &user2_id).await?;
    ...
    async fn find_by_ids_as_hash_map_does_not_return_other_users_authors(
        ...
        let result = author_repository
            .find_by_ids_as_hash_map(&mut conn, &user2_id, &[author_id])
            .await?;
        assert_eq!(result.len(), 0);
    ...
    async fn update_returns_not_found_for_nonexistent_author(pool: PgPool) -> anyhow::Result<()> {
        ...
        let result = author_repository.update(&mut conn, &user_id, &author).await;
    ...
    async fn update_returns_not_found_for_other_users_author(pool: PgPool) -> anyhow::Result<()> {
        ...
        let result = author_repository
            .update(&mut conn, &user2_id, &updated)
            .await;
        assert!(matches!(result, Err(DomainError::NotFound { .. })));
    ...
    async fn delete_fails_when_author_has_associated_books(pool: PgPool) -> anyhow::Result<()> {
        ...
        let result = author_repository
            .delete(&mut conn, &user_id, &author_id)
            .await;
        assert!(matches!(
            result,
    ...
    async fn delete_does_not_affect_other_users_when_book_association_blocks(
        ...
        author_repository
            .create(&mut conn, &user1_id, &author1)
            .await?;
        author_repository
        ...
        let result = author_repository
            .delete(&mut conn, &user2_id, &author_id)
            .await;
        assert!(matches!(
            result,
            Err(DomainError::HasAssociatedBooks { .. })
