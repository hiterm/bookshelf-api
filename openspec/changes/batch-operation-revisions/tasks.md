...
- [x] 2.1 Add typed Book and Author Revision batch keys and owner-scoped repository APIs that issue no SQL for empty inputs
- [x] 2.2 Implement paired-key PostgreSQL batch queries with duplicate, missing, cross-owner, mixed-key, and Book author-ID integration coverage
- [x] 2.3 Expose validated batch Revision lookups through HistoryQueryUseCase and test one batch repository call with correctly mapped DTO results

## 3. GraphQL Batch Loading

- [x] 3.1 Add and unit-test request-scoped BookRevisionLoader and AuthorRevisionLoader composite-key batching
- [x] 3.2 Register both Revision loaders and route all four Operation change Revision resolvers through the shared per-entity loaders while bypassing null keys
- [x] 3.3 Add GraphQL regression coverage for create/update, mixed entity types, missing sides, response compatibility, and many changes without per-field single lookups
