...
- [x] 1.1 Add explicit camelCase CurrentBackupV1 and FullBackupV1 DTOs, version envelopes, history/event DTOs, and serialization contract tests
- [x] 1.2 Implement format/version dispatch and current backup semantic validation with duplicate, value, and author-reference tests
...
- [x] 2.2 Add a narrow BackupRepository boundary and PostgreSQL read transactions that export current and full data from a consistent snapshot
- [x] 2.3 Implement atomic current replacement with preserved IDs/timestamps, retained history, and before/after snapshot_all events
...
- [x] 3.1 Add backup export/current restore/full restore use-case traits and interactors using Claims-derived UserId ownership
- [x] 3.2 Add REST handlers and JSON error mapping for malformed, unsupported, invalid, conflict, and internal failures
- [x] 3.3 Wire GET/POST current/full routes into dependency injection and the main router without changing GraphQL
- [x] 3.4 Enforce 10 MiB current and 100 MiB full restore body limits and return HTTP 413 for oversized payloads
...
- [x] 4.1 Add current backup round-trip E2E coverage including retained history and before/after snapshots
...
- [x] 4.4 Document current/full restore event semantics and shared transaction locking in event-recording architecture documentation
